...
export const vocabBatch64: VocabItem[] = [
  word('take', '잡다', 1, 'verb',
    ...
    tips({
      ...
      visual: '영화 촬영장에서 감독이 슬레이트를 치며 한 장면을 포착하는 모습을 상상해 보세요.',
      soundAlike: '테이크아웃(take-out) 커피를 손에 들고 나가는 모습을 떠올리면 기억하기 쉽습니다.',
      context: '명사로 쓰일 때 영화의 한 장면을 찍는 \'촬영분\'이나 특정 사안에 대한 개인의 \'의견\'을 뜻하기도 합니다.',
      synonymAntonym: '무언가를 얻는 것은 gain, 반대로 내어주거나 잃는 것은 loss와 대조됩니다.',
    }),
    {
      definition: 'the amount of something gained or an individual\'s particular perspective or interpretation of a situation',
      synonyms: ['opinion', 'viewpoint', 'catch'],
      antonyms: ['giving', 'loss', 'refusal'],
      exampleSentences: [
        { en: 'The director decided that the third attempt was the best version of the scene.', ko: '감독은 세 번째 촬영분이 해당 장면의 가장 좋은 버전이라고 결정했다.' },
        { en: 'What is your personal perspective on the new government policy?', ko: '새로운 정부 정책에 대한 당신의 개인적인 견해는 무엇입니까?' },
      ],
    }
  ),
  ...
  word('tap', '가볍게 두드리다', 6, 'verb',
    ...
    tips({
      ...
      context: '주로 무언가를 살짝 건드리거나 조심스럽게 다룰 때 사용되는 표현입니다.',
      synonymAntonym: '강하게 내리치는 것과는 반대로, 아주 미세하고 부드러운 힘을 주는 상황에 쓰입니다.',
    }),
    {
      definition: 'with a light and gentle touch or movement',
      synonyms: ['lightly', 'gently', 'softly'],
      antonyms: ['heavily', 'forcefully', 'strongly'],
      exampleSentences: [
        { en: 'She brushed the dust off the old book tap so as not to damage the pages.', ko: '그녀는 페이지가 상하지 않게 오래된 책의 먼지를 가볍게 털어냈습니다.' },
        { en: 'The rain fell tap against the window pane throughout the quiet evening.', ko: '조용한 저녁 내내 창문에 비가 가볍게 부딪히며 내렸습니다.' },
      ],
    }
  ),
  ...
  word('tattoo', '문신', 5, 'noun',
    ...
    tips({
      ...
      visual: '해 질 녘 군대 연병장에서 병사들이 막사로 돌아가도록 나팔수가 신호를 부는 모습을 상상해 보세요.',
      soundAlike: '타투(문신)와 발음이 같지만, 군대 맥락에서는 병사들을 불러 모으는 소리나 군악대의 공연을 뜻합니다.',
      context: '현대에는 군악대가 펼치는 화려한 행진이나 공연을 지칭할 때도 자주 사용되는 단어입니다.',
      synonymAntonym: '신호라는 뜻의 signal과 유사하며, 조용히 휴식하는 silence와는 반대되는 개념입니다.',
    }),
    {
      definition: 'A rhythmic tapping or drumming, or a bugle call sounded late in the evening as a signal for soldiers to return to their quarters.',
      synonyms: ['drumbeat', 'signal', 'bugle call'],
      antonyms: ['silence', 'quiet', 'stillness'],
      exampleSentences: [
        { en: 'The soldiers returned to their barracks immediately after hearing the evening tattoo.', ko: '병사들은 저녁 귀영나팔 소리를 듣자마자 즉시 막사로 복귀했다.' },
        { en: 'A rhythmic tattoo of drums echoed through the valley during the military ceremony.', ko: '군사 의례 동안 북소리의 리듬감 있는 울림이 계곡 전체에 메아리쳤다.' },
      ],
    }
  ),
  word('tax', '세금', 6, 'noun',
  ...
  word('teen', '십 대', 1, 'noun',
    ...
    tips({
      ...
      visual: '눈물을 흘리며 슬퍼하는 10대(teenager)의 이미지를 떠올려 보세요.',
      soundAlike: '숫자 10을 뜻하는 ten과 비슷하지만, 더 길게 발음하며 슬픈 감정을 실어보세요.',
      context: '현대 영어에서는 드물게 쓰이지만, 시나 고전 문학에서 깊은 비탄을 표현할 때 등장합니다.',
      synonymAntonym: 'sorrow와 같은 감정의 깊이를 공유하며, joy와는 정반대의 상태를 나타냅니다.',
    }),
    {
      definition: 'a state of mental suffering, grief, or misfortune',
      synonyms: ['sorrow', 'misery', 'grief'],
      antonyms: ['joy', 'happiness', 'delight'],
      exampleSentences: [
        { en: 'The old songs are filled with tales of love and teen.', ko: '그 오래된 노래들은 사랑과 슬픔의 이야기들로 가득 차 있다.' },
        { en: 'She could not hide the deep teen etched upon her face.', ko: '그녀는 얼굴에 새겨진 깊은 슬픔을 숨길 수 없었다.' },
      ],
    }
  ),
  word('teenage', '10대', 5, 'noun',
    ['성인', '유아', '노년', '중년', '학업', '직장', '결혼', '은퇴', '유년', '출생'],
    tips({
  ...
  word('terrace', '테라스', 9, 'noun',
    ...
    tips({
      ...
      visual: '산비탈에 층층이 쌓인 초록색 계단식 논의 풍경을 상상해 보세요.',
      soundAlike: '테라스(Terrace)가 있는 아파트가 층층이 쌓여 있는 모습을 연상하세요.',
      context: '농업이나 건축에서 경사면을 효율적으로 활용하기 위해 층을 낸 구조를 설명할 때 쓰입니다.',
      synonymAntonym: '계단식인 stepped와 유사하며, 평평한 지형인 flat과는 반대되는 개념입니다.',
    }),
    {
      definition: 'formed into a series of flat areas resembling steps on a slope',
      synonyms: ['stepped', 'graduated', 'tiered'],
      antonyms: ['flat', 'level', 'smooth'],
      exampleSentences: [
        { en: 'Farmers cultivated rice on the steep hillsides using a terrace system.', ko: '농부들은 계단식 시스템을 사용하여 가파른 산비탈에서 쌀을 재배했습니다.' },
        { en: 'The ancient civilization built magnificent terrace gardens that overlooked the valley.', ko: '고대 문명은 계곡이 내려다보이는 웅장한 계단 모양의 정원을 건설했습니다.' },
      ],
    }
  ),
  ...
  word('terrific', '무서운', 6, 'adjective',
    ...
    tips({
      ...
      visual: '천둥 번개가 치는 거대한 폭풍우 속에서 느끼는 압도적인 공포와 경외심을 상상해 보세요.',
      soundAlike: '발음이 \'테러(terror)\'와 비슷하므로, 테러를 당했을 때 느끼는 극심한 공포를 연상하면 쉽습니다.',
      context: '현대 영어에서는 주로 \'아주 좋은\'으로 쓰이지만, 문학적 맥락에서는 여전히 소름 끼치도록 무서운 상황을 묘사할 때 사용됩니다.',
      synonymAntonym: '무서운이라는 뜻일 때는 terrifying과 비슷하며, 반대로 안심을 주는 상황은 soothing이라고 할 수 있습니다.',
    }),
    {
      definition: 'causing great fear, dread, or awe due to being very intense or overwhelming',
      synonyms: ['terrifying', 'dreadful', 'frightful'],
      antonyms: ['calming', 'pleasant', 'soothing'],
      exampleSentences: [
        { en: 'The mountain climbers faced a terrific storm that threatened their very lives.', ko: '산악인들은 그들의 생명을 위협하는 무시무시한 폭풍에 직면했습니다.' },
        { en: 'A terrific explosion shook the entire building to its foundations.', ko: '무시무시한 폭발이 건물 전체를 기초부터 흔들어 놓았습니다.' },
      ],
    }
  ),
