...
export const vocabBatch38: VocabItem[] = [
  ...
  word('haunt', '자주가다', 8, 'noun',
    ...
    tips({
      ...
      visual: '단골 손님이 카페 구석 자리에 매일 앉아 있는 편안하고 익숙한 풍경을 상상해 보세요.',
      soundAlike: '\'헌트(hunt)\'와 발음이 비슷하지만, 사냥하러 다니는 것이 아니라 단골집을 찾아다니는 느낌으로 기억하세요.',
      context: '주로 예술가들이나 문학가들이 영감을 얻기 위해 자주 찾는 아지트 같은 장소를 묘사할 때 쓰입니다.',
      synonymAntonym: '자주 가는 곳을 뜻하는 hangout과 유사하며, 한 번도 가보지 않은 낯선 곳과는 반대되는 개념입니다.',
    }),
    {
      definition: 'A place that is frequently visited by a particular person or group of people.',
      synonyms: ['hangout', 'resort', 'den'],
      antonyms: ['void', 'wilderness', 'outpost'],
      exampleSentences: [
        { en: 'The old library became a favorite haunt for students seeking a quiet place to study.', ko: '그 오래된 도서관은 조용히 공부할 곳을 찾는 학생들에게 인기 있는 아지트가 되었다.' },
        { en: 'Local musicians often gather at this riverside cafe, which has been their primary haunt for years.', ko: '지역 음악가들은 수년 동안 그들의 주요 단골 장소였던 이 강변 카페에 자주 모인다.' },
      ],
    }
  ),
  word('have', '가지', 2, 'noun',
